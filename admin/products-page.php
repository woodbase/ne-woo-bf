...
<table class="widefat striped nebf-products-table">
    ...
    <tbody>
        <?php foreach ($products_page as $i => $product):
            $existing = get_posts(['post_type'=>'product','meta_key'=>'_beautyfort_id','meta_value'=>$product['bf_id'],'fields'=>'ids']);
            $is_imported = !empty($existing);
            $accordion_id = 'acc-' . $i;
        ?>
        ...
        <tr id="<?= esc_attr($accordion_id); ?>" class="accordion-row" style="display:none;">
            <td colspan="9" style="background:#f6f7f7; padding:16px;">
                <table class="widefat striped" style="margin:0;">
                    <tbody>
                        <tr><th>Beskrivning</th><td><?= nebf_format_field($product['description'] ?? null); ?></td></tr>
                        <tr><th>EAN</th><td><?= nebf_format_field($product['barcode'] ?? null); ?></td></tr>
                        <tr><th>Ingredienser</th><td><?= nebf_format_field($product['ingredients'] ?? null); ?></td></tr>
                        <tr><th>Volym</th><td><?= nebf_format_field($product['size'] ?? null); ?></td></tr>
                        <tr><th>Färg</th><td><?= nebf_format_field($product['color'] ?? null); ?></td></tr>
                        <tr><th>Övrigt</th><td><?= nebf_format_field($product['extra_info'] ?? null); ?></td></tr>
                        <tr><th>BF-ID</th><td><?= nebf_format_field($product['bf_id'] ?? null); ?></td></tr>
                        <tr><th>SKU</th><td><?= nebf_format_field($product['sku'] ?? null); ?></td></tr>
                    </tbody>
                </table>
